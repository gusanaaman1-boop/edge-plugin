...
# load. Passing our 226 checks and failing this would still mean a plug-in that
...
fail=0
for p in "$HOME/Library/Audio/Plug-Ins/VST3/EDGE.vst3" \
         "$HOME/Library/Audio/Plug-Ins/Components/EDGE.component"; do
    echo
    echo "== $(basename "$p") =="
    if [ ! -e "$p" ]; then
